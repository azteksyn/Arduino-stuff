...
#define MIN_BRIGHTNESS 12       // set to a minimum of 25%


 

const int brightnessInPin = A0;  // The Analog input pin that the brightness control potentiometer is attached to.
const int speedInPin = A1;       // The Analog input pin that the speed control potentiometer is attached to.
...
void setup() {
  ...
  FastLED.addLeds<LED_TYPE,DATA_PIN,COLOR_ORDER>(leds, NUM_LEDS).setCorrection(TypicalLEDStrip); // Setup the led strip
...
typedef void (*SimplePatternList[])(); // sets up a simple playlist of patterns and plays one after the other sequencially 
...
//SimplePatternList gPatterns = {rainbowb};
uint8_t gCurrentPatternNumber = 0; // Index number of which pattern is current

uint8_t gHue = 0; // rotating "base color" used by many of the patterns
uint8_t gHue2 = 0;// rotating "base color" used by many of the patterns

uint8_t gHueDelta = 0.1;  //The speed of whitch the colour pallet is cycled through, negative numbers reverse the direction + forward - backward
uint8_t gHueDelta2 = -3;

/////////////////////////////////////////////////////////////////////////////////
  
void loop()
{
 int mappedValue = map(analogRead(brightnessInPin), 0, 1023, 0, 255); //code that sets
 FastLED.setBrightness(constrain(mappedValue, MIN_BRIGHTNESS, MAX_BRIGHTNESS));
 int mappedHue;  
 
 int FRAMES_PER_SECOND = map(analogRead(speedInPin), 0, 1023, 2, 120);
  ...
  EVERY_N_MILLISECONDS( 70 ) { gHue++; } // slowly cycle the "base color" through the rainbow
  EVERY_N_MILLISECONDS( 10 ) { gHue2++; }
...
void rainbowc() 
{
  fill_rainbow( leds, NUM_LEDS, gHue2, 1);
  gHue2 += gHueDelta2;
}
...
void juggle()
{
  ...
  for( int i = 0; i < 8; i++)
  {
    leds[beatsin16(i + 1, 0, NUM_LEDS)] |= CRGB::White;
    dothue += 32;
  }
}
//////////////////////////////////////////////////////////////////////////
